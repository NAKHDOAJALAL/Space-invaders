...
class Game:
    def __init__(self):
        ...
        self.live_surf = pygame.image.load(
            'Space-invaders-main/graphics/player.png').convert_alpha()
        self.live_x_start_pos = screen_width - \
            (self.live_surf.get_size()[0] * 2 + 20)
        self.score = 0
        self.font = pygame.font.Font(
            'Space-invaders-main/font/Pixeled.ttf', 20)

        # obstavle setup
        self.shape = obstacle.shape
        self.block_size = 6
        self.blocks = pygame.sprite.Group()
        self.obsticle_amont = 4
        self.obsticle_x_positions = [
            num * (screen_width / self.obsticle_amont) for num in range(self.obsticle_amont)]
        self.create_multiple_obsticles(
            *self.obsticle_x_positions, x_start=screen_width / 15, y_start=480)
        ...
        self.laser_sound = pygame.mixer.Sound(
            'Space-invaders-main/audio/laser.wav')
        self.laser_sound.set_volume(0.03)
        self.explosion_sound = pygame.mixer.Sound(
            'Space-invaders-main/audio/explosion.wav')
        self.explosion_sound.set_volume(0.03)

    def create_obstalce(self, x_start, y_start, offset_x):
    ...
    def create_multiple_obsticles(self, *offset, x_start, y_start):
        for offset_x in offset:
            self.create_obstalce(x_start, y_start, offset_x)
...
class CRT:
    def __init__(self):
        self.tv = pygame.image.load(
            'Space-invaders-main/graphics/tv.png').convert_alpha()
